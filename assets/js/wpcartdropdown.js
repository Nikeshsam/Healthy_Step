...
class WebiQuantityInput extends HTMLElement {
    constructor() {
        super(), (this.input = this.querySelector("input")), (this.changeEvent = new Event("change", { bubbles: !0 })), this.querySelectorAll("button").forEach((button) => button.addEventListener("click", this.onButtonClick.bind(this)));
    }
    onButtonClick(event) {
    ...
    constructor() {
        super(),
            this.addEventListener("click", (event) => {
                event.preventDefault(), this.closest("webi-cart-items").updateQuantity(this.dataset.index, 0);
            });
    }
}
...
class WebiCartItems extends HTMLElement {
    constructor() {
        super(),
            ...
            (this.currentItemCount = Array.from(this.querySelectorAll('[name="updates[]"]')).reduce((total, webiquantityInput) => total + parseInt(webiquantityInput.value), 0)),
    ...
    getSectionsToRender() {
        return [
            { id: "webi-main-cart-items", section: document.getElementById("webi-main-cart-items").dataset.id, selector: ".js-contents" },
            { id: "cart-icon-bubble", section: "cart-icon-bubble", selector: ".shopify-section" },
            { id: "webi-cart-live-region-text", section: "webi-cart-live-region-text", selector: ".shopify-section" },
            { id: "webi-main-cart-footer", section: document.getElementById("webi-main-cart-footer").dataset.id, selector: ".js-contents" },
        ];
    }
    updateQuantity(line, quantity, name) {
        ...
        fetch(`${routes.cart_change_url}`, { ...fetchConfig(), body })
            ...
            .then((state) => {
                ...
                lineItem && lineItem.querySelector(`[name="${name}"]`) && lineItem.querySelector(`[name="${name}"]`).focus(), this.disableLoading();
            })
            .catch(() => {
                this.querySelectorAll(".loading-overlay").forEach((overlay) => overlay.classList.add("hidden")), this.disableLoading();
            });
    }
    updateLiveRegions(line, itemCount) {
        this.currentItemCount === itemCount &&
            (document.getElementById(`Wb-Line-item-error-${line}`).querySelector(".wb-cart-item__error-text").innerHTML = window.cartStrings.quantityError.replace("[quantity]", document.getElementById(`WbQuantity-${line}`).value)),
            (this.currentItemCount = itemCount),
...
//# sourceMappingURL=/cdn/shop/t/2/assets/wbcartdropdown.js.map?v=168107083126735237411663905902
